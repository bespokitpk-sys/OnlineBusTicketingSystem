...
echo "<h2>Updating Database for Module 3...</h2>";

// Add trip_status column to schedules table if it doesn't exist
$result = $conn->query("SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'schedules' AND COLUMN_NAME = 'trip_status'");
...
$result = $conn->query("SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'tickets' AND COLUMN_NAME = 'boarded_at'");
...
$result = $conn->query("SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'schedules' AND COLUMN_NAME = 'operator_id'");

if ($result->num_rows == 0) {
    $conn->query("ALTER TABLE schedules ADD COLUMN operator_id INT NULL AFTER bus_id, ADD FOREIGN KEY (operator_id) REFERENCES users(id) ON DELETE SET NULL");
    echo "✅ Added operator_id column to schedules table<br>";
} else {
